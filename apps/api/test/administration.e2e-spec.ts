  ...
  describe('user write', () => {
    ...
    it('rejects a duplicate email (409)', async () => {
      await request(app.getHttpServer())
        .post('/api/v1/users')
        .set(bearer(await token('admin@hsdg.in')))
        .send({ email: 'partner.a@hsdg.in', displayName: 'Dupe', officeCode: 'NORTH' })
        .expect(409);
    });
    ...
    it('rejects an unknown office (400)', async () => {
      await request(app.getHttpServer())
        .post('/api/v1/users')
        .set(bearer(await token('admin@hsdg.in')))
        .send({ email: `qa.${uniq()}@hsdg.in`, displayName: 'X', officeCode: 'NOPE' })
        .expect(400);
    });
    ...
    it('forbids a Partner (lacks user.manage) from creating a user (403)', async () => {
      await request(app.getHttpServer())
        ...
        .send({ email: `qa.${uniq()}@hsdg.in`, displayName: 'X', officeCode: 'NORTH' })
        .expect(403);
    });
    ...
    it('rejects an unknown role slug (400)', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/users')
        .set(bearer(await token('admin@hsdg.in')))
        .send({ email: `qa.${uniq()}@hsdg.in`, displayName: 'Bad Role', officeCode: 'NORTH' })
        .expect(201);
      await request(app.getHttpServer())
        .put(`/api/v1/users/${created.body.id}/roles`)
        .set(bearer(await token('admin@hsdg.in')))
