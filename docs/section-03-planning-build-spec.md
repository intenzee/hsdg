...
| 03.4 Audit Scope & Approach — 03.4.1 Scope Intelligence (read-only from 02.1/02.2–02.7/03.1–03.3, 7 landing cards), SC-01/SC-02 (prefilled from 02.6; correction routes to Section 02), Audit Population generated idempotently from engagement/02.6 investees/branches/service org + manual units (no TB; unit indicators with source; materiality shown as "× OM" context only — never an exclusion; SC-03 relevance; auditor + other-auditor strategy; scope conclusion with rationale gate; "significance not yet available" auto-creates a dependency), AP-01 (Not Yet Determinable kept) + §8 strategy prompts, controls reliance by cycle (Reliance Contemplated / No Reliance / Further Assessment — never "effective"), interim/YE timing + roll-forward prompts (no dates), 13 evidence channels, EC-01 with suggested confirmation areas, SA 501 inventory attendance (owned-dependency rule), OB-01 (auto on initial audit), SO-01 cards, IA-01, joint audit, specialists from 03.8-routed signals (Evaluate Further → Planning Matter; Not Required on Enhanced/IPA needs rationale), §20 special implications from signals/facts, DT-01 + technology uses, Scope Dependency Register (significant impact → Partner Attention), SL-01 limitations (always IPA + open Planning Matter; no report-modification judgment), Preliminary Audit Approach Map (auto areas from 03.2 metrics + CFS/initial-audit, live amounts, specific-materiality hints, controls/timing/evidence/special, removal needs reason), §24 consistency checks (blocking), §25 generated conclusion, AP-02 completion gate, §26 Partner Planning View actions (Agree/Challenge/Request/Add IPA signal → 03.12), §27 controlled revision v1.0→v1.1 with frozen baseline JSON, Reassessment Required from Section 05 (endpoint) and 03.3 materiality-revision completion (hook) | ✅ built, live-tested end-to-end (v1.0 complete → Section 05 reassessment → v1.1 revision) | migration `1764600000000_statutory_audit_scope_approach.sql`; contract `statutory-audit-scope-approach.ts`; `scope-approach-engine.ts` (pure, unit-tested), `audit-scope-approach.{service,controller}.ts`, `dto/scope-approach.dto.ts`; web `scope-approach-panel.tsx` (mounted on the `audit_approach` row; `overall_audit_plan` row follows 03.4's status) |
| 03.4 still open | ⏳ | Strategic taxonomy (cycles, evidence channels, consistency rules) is a versioned constant in code (`dhvaj-scope-2026.1`), not a methodology-admin library; Section 05 doesn't exist yet — its "reliance unsupported" trigger is the `scope-approach/reassessment` endpoint for it to call; the 03.3 → 03.4 materiality hook was tested via the same function, not by completing a real 03.3 revision; component/branch list relies on 02.6 investees + a single "branches" unit (no per-branch master in 02.6); SharePoint evidence not linked; Partner actions are open to any engagement lead (EP-only split not enforced, same as 03.1–03.3) |
| 03.5 Audit Areas & Assertions (FROZEN spec) — versioned Audit Area Library (`audit_area_library_release` + `audit_area_library`, §6 fields, §7 General Corporate baseline seeded as data `dhvaj-gc-2026.1`, 54 areas incl. CFS-conditional Consolidation and initial-audit Opening Balances) + canonical assertion master (`audit_assertion`, 13 SA 315 IDs exactly); §4 applicable-library selection (FRF from 02.2 / Ind AS assessment, CFS, initial audit, effective period); §3 population created once on first entry (all Retained, idempotent, library version + profile stored); §5 landing tiles/filters/alias search/indicators (Financial Data, Planning Signal, Specific Materiality, Statutory, CFS, CARO, Prior-Year Area); §8 engagement area record (snapshot, never deleted); §9 remove (5 reason codes, covered-under must be another retained area, rationale when any warning exists) / restore (reinstates assertions); §10 contextual warnings (never auto-restore); §11 Add from Library (duplicate → Restore) / Custom (name/type/reason, engagement-only); §13 live 03.2 amounts or optional manual amount (currency + unit), Above/Below OM informational only; §14 attention Standard / Enhanced / Requires Review (downgrade of portal suggestion needs reason; never a risk rating); §16–17 assertions keep/add/remove(reason)/restore + assertion-level Enhanced suggestions from library scenario rules; §18 signal auto-link via library mapping tags + manual link/unlink (inactive kept); VAL-04 specific-materiality mapping (keyed by matter name so it survives 03.3 revisions); VAL-05 "no Audit Area mapping required" resolution; §19 framework panel from Authority Library (AS vs Ind AS filtered); §20 prior-year context via predecessor engagement; VAL-01…VAL-10 machine-readable; §22 AA-01 exact text → immutable matrix snapshot per version (insert-only table); §23 matrix (retained + active assertions only); §25 impact rules against a stored completion basis (materiality decrease between thresholds, new specific, new Enhanced/IPA signal, CFS / initial-audit change, new financial area) → Update Required (derived live + persisted by 03.3 completion and 03.1 signal create/assess hooks), reopen with reason → next version with impacted areas flagged Requires Review; controlled methodology refresh; §27 Partner comments / challenges / reassessment requests; full audit trail | ✅ built, live-tested end-to-end (v1 → v2 → v3, Update Required from materiality decrease and new signals) | migration `1764700000000_statutory_audit_areas_assertions.sql`; contract `statutory-audit-areas-assertions.ts`; `area-review-engine.ts` (pure, unit-tested), `audit-area-review.{service,controller}.ts`, `dto/area-review.dto.ts`; web `audit-areas-panel.tsx` (mounted on the `areas_and_assertions` row) |
| 03.5 still open | ⏳ | Library **default assertions, signal-mapping tags, §17 scenario rules and indicator tags are provisional methodology data** seeded by us — DHVAJ to confirm (append a new library release; no admin UI yet); only a General Corporate profile exists (no Ind AS-only / industry additions supplied); §9.1 "removal blocked by downstream work" is a stub returning 0 until 03.6 exists; SharePoint supporting files (§28) not linked; Section 02 CFS / initial-audit corrections are detected live (derived Update Required) but have no hook to persist status / checklist roll-up; Partner vs Manager rights not split (any engagement lead); prior-year context needs the predecessor engagement to have a 03.5 in the portal |
...
- 03.4 keeps ONE record per audit file with `version_no`; a completed (or reassessment-required) record is immutable (409) and a revision snapshots the full approved state into `audit_scope_revision.baseline` before reopening as the next version. Reassessment is only ever *flagged* (status + per-map-item flags) — never an automatic change of a decision. 03.4 also rolls up the `overall_audit_plan` checklist row.
